  ...
  if (!user) {
    return (
      <div className="max-w-md mx-auto px-4 py-20 text-center space-y-4">
        ...
        <h2 className="font-playfair text-3xl font-bold text-ceylon-950">Sign In to My Sri Lanka</h2>
    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-10 space-y-8 pb-20">
      <div className="glass-panel p-8 rounded-3xl border border-ceylon-500/30 flex flex-col sm:flex-row items-center justify-between gap-6">
        <div className="flex items-center space-x-4">
          <div className="w-16 h-16 rounded-full bg-ceylon-600 border-2 border-ceylon-400 flex items-center justify-center text-ceylon-950 font-bold text-xl">
            {user.name?.substring(0, 2).toUpperCase() || 'US'}
          </div>
          <div>
            <h1 className="font-playfair text-2xl sm:text-3xl font-bold text-ceylon-950">{user.name}</h1>
            <p className="text-xs text-ceylon-700">{user.email} • {user.role}</p>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-3">
          ...
          <button onClick={logout} className="p-2.5 rounded-full glass-card hover:text-red-400 text-ceylon-700 transition">
      ...
      <div className="flex space-x-4 border-b border-ceylon-500/20 pb-2">
        <button onClick={() => setTab('saved')} className={`px-4 py-2 font-semibold text-xs uppercase tracking-wider ${tab === 'saved' ? 'text-ceylon-300 border-b-2 border-ceylon-400' : 'text-ceylon-700'}`}>
          Saved Places ({savedIds.length})
        </button>
        <button onClick={() => setTab('trips')} className={`px-4 py-2 font-semibold text-xs uppercase tracking-wider ${tab === 'trips' ? 'text-ceylon-300 border-b-2 border-ceylon-400' : 'text-ceylon-700'}`}>
          My Itineraries
        </button>
      </div>
      ...
      {tab === 'saved' && (
        <div className="space-y-4">
          {savedIds.length === 0 ? (
            <div className="p-12 text-center glass-panel rounded-2xl text-ceylon-700 text-xs">
              No saved items yet. Browse destinations or experiences and click the bookmark icon to save them here!
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {savedIds.map((id) => (
                <div key={id} className="glass-card p-4 rounded-xl flex items-center justify-between text-xs text-ceylon-950">
      ...
      {tab === 'trips' && (
        <div className="space-y-4">
          <div className="glass-card p-6 rounded-2xl flex flex-col sm:flex-row items-center justify-between gap-4">
            <div>
              <h4 className="font-playfair text-lg font-bold text-ceylon-950">7-Day Ceylon Essential Adventure</h4>
              <p className="text-xs text-ceylon-700">Sigiriya • Kandy • Ella • Yala • Mirissa</p>
            </div>
