    ...
    <div className="bg-white text-ceylon-950 pb-20">
      <section className="relative min-h-[56vh] pt-32 flex items-end overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 pb-12 w-full">
          <Link href="/map" className="inline-flex items-center gap-2 text-sm font-semibold text-ceylon-100 hover:text-white transition">
            <ArrowLeft className="w-4 h-4" />
            <span>Back to Map</span>
          </Link>
          <div className="mt-6 max-w-3xl">
            <p className="text-xs uppercase tracking-widest text-ceylon-200 font-bold">Province Guide</p>
            <h1 className="font-playfair text-5xl sm:text-7xl font-bold text-white mt-2">
              {province.name}
            </h1>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 py-14">
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-8">
          ...
          <Link
            ...
            className="inline-flex items-center justify-center gap-2 rounded-full bg-ceylon-700 px-6 py-3 text-xs font-bold uppercase tracking-wider text-white hover:bg-ceylon-600 transition"
          >
          ...
          {province.districts.map((district) => (
            <article key={district.name} className="rounded-2xl border border-ceylon-950/10 bg-white p-6 shadow-xl shadow-ceylon-950/5">
              <div className="flex items-start justify-between gap-4">
                ...
                <span className="h-10 w-10 rounded-full flex items-center justify-center text-white" style={{ backgroundColor: province.color }}>
              ...
              <div className="mt-5 space-y-3">
                {district.famousLocations.map((location, index) => (
                  <div key={location} className="flex items-center justify-between gap-4 rounded-xl border border-ceylon-950/10 px-4 py-3">
                    <span className="flex items-center gap-3 min-w-0">
                      <span className="text-xs font-bold text-ceylon-500">{String(index + 1).padStart(2, '0')}</span>
                      <span className="font-semibold text-ceylon-950">{location}</span>
                    </span>
                    <MapPin className="w-4 h-4 text-ceylon-600 flex-shrink-0" />
                  </div>
                ))}
              </div>
            </article>
